...
import { h, icon, isHost, kbd } from "./root";
...
export function createOverlay(): Overlay {
  ...
  const parentBtn = h("button", { class: "parent", onclick: () => void executeCommand("select_parent", {}) }, icon("parent"));
  const actions = h(
    "div",
    { class: "actions" },
    handle,
    h("span", { class: "sep" }),
    parentBtn,
    h("button", { class: "edit", title: "Edit text (or double-click)", onclick: () => onEdit() }, icon("edit"), h("span", {}, "Edit text")),
    h("button", { class: "danger", title: "Remove", onclick: () => onRemove() }, icon("remove"), h("span", {}, "Remove")),
    h("button", { class: "note", title: "Note", onclick: () => onNote() }, icon("note"), noteLabel),
  );
  const dotSep = () => h("span", { class: "dot" });
  const editHint = h("span", { class: "hint" }, kbd("↵"), " Save", dotSep(), kbd("Esc"), " Cancel", dotSep(), kbd("⇧↵"), " New line");
  const bar = h("div", { class: "bar", role: "toolbar", "aria-label": "Reword actions" }, actions, editHint);
  ...
  const noteBox = h(
    ...
    h(
      "div",
      { class: "row" },
      h("span", { class: "spacer" }, kbd(/Mac|iPhone|iPad/.test(navigator.platform) ? "⌘" : "Ctrl"), kbd("↵")),
      noteDelete,
  ...
  const updateHandle = () => {
    ...
    // Clicked too deep (the <span> inside a button)? The parent button widens the selection.
    const parent = sel?.parentElement;
    const hasParent = !!parent && parent !== document.body && parent !== document.documentElement;
    parentBtn.style.display = hasParent ? "" : "none";
    if (hasParent) {
      parentBtn.title = `Select the parent <${parent.localName}>`;
      parentBtn.setAttribute("aria-label", parentBtn.title);
      parentBtn.replaceChildren(icon("parent"), h("span", { class: "t-tag" }, parent.localName));
    }
  };
