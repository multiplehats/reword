...
import { h, icon, kbd, type IconName } from "./root";

const PANEL_KEY = "reword:panel";
...
const KIND_ICON: Record<Change["type"], IconName> = { edit: "edit", remove: "remove", note: "note", move: "move" };

export function createPanel({ prefs, siteToggle }: Pick<RewordOptions, "prefs" | "siteToggle">) {
  const count = h("span", { class: "count" }, "0");
  const collapseBtn = h("button", { class: "icon-btn collapse", title: "Collapse", "aria-label": "Collapse panel" }, icon("chevron"));
  const closeBtn = h("button", { class: "icon-btn", title: "Close (Alt+Shift+E). Your changes are kept.", "aria-label": "Close" }, icon("close"));
  const head = h(
    "div",
    { class: "head" },
    h("span", { class: "logo" }, icon("mark")),
    h("span", { class: "name" }, "Reword"),
    count,
  ...
  const undoBtn = h("button", { class: "btn quiet", title: `Undo (${MOD}+Z)` }, icon("undo"), h("span", {}, "Undo"));
  const redoBtn = h("button", { class: "btn quiet", title: `Redo (${MOD}+Shift+Z)` }, icon("redo"), h("span", {}, "Redo"));
  const clearBtn = h("button", { class: "btn danger" }, "Clear all");
  const copyBtn = h("button", { class: "btn primary copy", title: `Copy prompt (${MOD}+Shift+C)` });
  const siteBox = h("input", { type: "checkbox" });
  const siteRow = siteToggle
    ? h("label", { class: "site", title: `Press E on ${location.hostname} to open or close Reword` }, siteBox, "Open with ", kbd("E"), " on this site")
    : null;
  if (siteToggle) {
    void siteToggle.get().then((on) => (siteBox.checked = on), () => {});
    siteBox.addEventListener("change", () => void siteToggle.set(siteBox.checked));
  }
  ...
  const body = h(
    ...
    copyBtn,
    copyHelp,
    h("div", { class: "foot" }, h("span", {}, "Hold ", kbd("Alt"), " to use the page normally"), siteRow),
  );
  const panel = h("div", { class: "panel", role: "region", "aria-label": "Reword changes" }, head, body);
  ...
  function setPanel(patch: Partial<State["panel"]>) {
    ...
    void prefs?.set(PANEL_KEY, next).catch(() => {});
  }
  ...
  function render(s: State) {
    ...
    copyBtn.replaceChildren(
      h("span", { class: "copy-icon" }, icon(copyState ? (copyState.kind === "done" ? "check" : "warn") : "copy")),
    ...
    if (!n) {
      list.replaceChildren(
        ...
        h(
          ...
          h("div", { class: "e-hint" }, "Double-click to edit text right away. Clicked a word inside a button? Use ", icon("parent"), " to select the button."),
        ),
      );
      return;
    }
    ...
    s.changes.forEach((c, i) => {
      ...
      const x = h("button", { class: "x", title: "Revert this change", "aria-label": `Revert change ${i + 1}` }, icon("close"));
      ...
      const item = h(
        ...
        h(
          ...
          h(
            "div",
            { class: `kind k-${c.type}` },
            h("span", { class: "k-icon" }, icon(KIND_ICON[c.type])),
            KIND_LABEL[c.type],
  ...
  async function loadPosition() {
    const saved = (await prefs?.get(PANEL_KEY).catch(() => undefined)) as State["panel"] | undefined;
    if (saved)
