// Page event interception while Reword is open. Clicks are swallowed in the
...
import { isHost, isOurEvent } from "./root";
...
function pageTarget(e: Event): Element | null {
  ...
  return me.clientX === undefined ? t : throughEmptyLayer(t, me.clientX, me.clientY);
}

const MEDIA = "img,svg,video,picture,canvas,iframe,input,textarea,select";

/**
 * Framer, Webflow and friends often lay empty links or click-catchers over the
 * copy. When the pointer is on such an empty layer, target the content
 * painted underneath it instead (not one of the layer's own ancestors).
 */
function throughEmptyLayer(t: Element, x: number, y: number): Element {
  if (t.textContent?.trim() || t.matches(MEDIA) || t.querySelector(MEDIA)) return t;
  for (const el of document.elementsFromPoint(x, y)) {
    if (el === t || el.contains(t) || isHost(el) || el === document.body || el === document.documentElement) continue;
    if (el.textContent?.trim() || el.matches(MEDIA)) return el;
  }
  return t;
}
...
export function createInterceptor(overlay: Overlay) {
  ...
  function onKeyDown(e: KeyboardEvent) {
    ...
    if (mod && e.shiftKey && !e.altKey && key === "c") {
      // Chrome may keep ⌘⇧C for DevTools while DevTools is open; the panel button always works.
      e.preventDefault();
      e.stopImmediatePropagation();
      void executeCommand("copy_prompt");
      return;
    }
