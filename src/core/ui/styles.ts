...
:host {
  ...
  pointer-events: none !important; display: block !important; contain: layout style !important;
}
...
.flash.show { animation: rw-flash 1s var(--ease-out) forwards; }
@keyframes rw-flash { 0%, 45% { opacity: 1; } 100% { opacity: 0; } }
...
.bar .parent { gap: 4px; padding: 0 7px; }
.bar .parent .t-tag { font: 500 11px/1 var(--font-mono); color: var(--color-ink-3); }
...
.panel .empty .e-hint { display: block; margin-top: 6px; color: var(--color-ink-3); font-size: 12px; }
...
.panel .foot { display: flex; flex-direction: column; gap: 6px; padding: 8px 12px 11px; color: var(--color-ink-3); font-size: 11.5px; }
.panel .foot > span, .panel .site { display: flex; align-items: center; gap: 5px; }
.panel .site { cursor: pointer; color: var(--color-ink-2); }
.panel .site input { margin: 0 2px 0 0; accent-color: var(--color-accent); }
.panel .empty .e-hint svg { display: inline-block; vertical-align: -2px; }
.logo .i-bg { fill: var(--color-bg-raised); }
.logo .i-ink { fill: var(--color-ink); }
.logo .i-accent { fill: var(--color-accent); }
