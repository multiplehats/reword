...
const IGNORED_DATA = /^data-(v-|reactid|react|radix|headlessui|state|orientation|side|align|aria|rk|framer|sentry|gtm|ga|analytics|track|ved|hveid|view-component|turbo|n-|nimg|nextjs|styled|emotion|reword)/;
...
/**
 * The element's copy as written in source: `text-transform` is ignored (an
 * `uppercase` badge reads "New feature"), and screen-reader-only and hidden
 * text is skipped. Used for everything that ends up in the prompt.
 */
export function textOf(el: Element): string {
  return extractText(el).text;
}

/** Text as rendered (innerText). Cheaper than textOf; used for matching search queries. */
...
/** Text-node segments, when the copy is split across elements. Empty when it's one run. */
export function segmentsOf(el: Element): string[] {
  const { segments } = extractText(el);
...
export function sourceHints(el: Element): string[] {
  ...
  for (let depth = 0; cur && cur !== document.body && depth < 8; depth++, cur = cur.parentElement) {
    for (const a of HINT_ATTRS) {
      const v = cur.getAttribute(a);
      if (v && hints.length < 6) {
        const hint = `${a}="${truncate(v, 80)}"`;
